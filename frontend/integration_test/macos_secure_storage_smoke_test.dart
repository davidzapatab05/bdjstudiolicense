...
void main() {
  ...
  group('macOS Hosted Runner — App Launch + Native Keychain Smoke Test', () {
    testWidgets('Verify write, read, overwrite, persist, and fail-closed delete on Keychain', (WidgetTester tester) async {
      ...
      final testKey = 'bdj.license.ci_test.${DateTime.now().microsecondsSinceEpoch}';
    ...
    testWidgets('Verify secure storage for fictional administrative operator test data (License Console Only)', (WidgetTester tester) async {
      print('[CI_TEST_HOST_ADMIN] Booting minimal UI host for Administrative Keychain test...');
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: Center(
              child: Text('BDJ Administrative Keychain CI Test Host'),
            ),
          ),
        ),
      );
      await tester.pumpAndSettle();
      print('[CI_TEST_HOST_ADMIN] Minimal Flutter UI mounted successfully!');
      ...
      final privKeyName = 'bdj.license.ci_test.op_private_key_$timestamp';
      final pubKeyName = 'bdj.license.ci_test.op_public_key_$timestamp';
      final certName = 'bdj.license.ci_test.op_cert_$timestamp';

      const fictionalPrivKey = 'FICTIONAL_PRIVATE_KEY_FOR_TEST_ONLY_DO_NOT_USE_IN_PROD_12345';
      const fictionalPubKey = 'FICTIONAL_PUBLIC_KEY_FOR_TEST_ONLY_67890';
      const fictionalCert = 'FICTIONAL_CERTIFICATE_PEM_TEST_DATA_ABCDE';

      try {
        print('[KEYCHAIN_ADMIN_TEST] Writing fictional administrative operator credentials...');
        await storage.write(key: privKeyName, value: fictionalPrivKey, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        await storage.write(key: pubKeyName, value: fictionalPubKey, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        await storage.write(key: certName, value: fictionalCert, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        print('[KEYCHAIN_ADMIN_TEST] Fictional credentials written successfully to native Keychain.');

        print('[KEYCHAIN_ADMIN_TEST] Reading back and verifying integrity...');
        final readPriv = await storage.read(key: privKeyName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        final readPub = await storage.read(key: pubKeyName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        final readCert = await storage.read(key: certName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);

        expect(readPriv, equals(fictionalPrivKey));
        expect(readPub, equals(fictionalPubKey));
        expect(readCert, equals(fictionalCert));
        print('[KEYCHAIN_ADMIN_TEST] All fictional administrative items read and verified correctly!');

      } catch (e) {
        print('=== ERROR IN ADMINISTRATIVE OPERATOR KEYCHAIN TEST ===');
        print('Status code: KEYCHAIN_WRITE_FAILED / KEYCHAIN_READ_FAILED');
        print('Exception (sanitized): ${e.toString().replaceAll("FICTIONAL", "[REDACTED]")}');
        rethrow;
      } finally {
        print('[KEYCHAIN_ADMIN_TEST] Cleaning up all fictional administrative keys from Keychain...');
        await storage.delete(key: privKeyName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        await storage.delete(key: pubKeyName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        await storage.delete(key: certName, aOptions: androidOptions, iOptions: iOsOptions, mOptions: macOsOptions, wOptions: windowsOptions, lOptions: linuxOptions);
        print('[KEYCHAIN_ADMIN_TEST] Fictional administrative test keys cleaned up successfully.');
      }
    });
  });
}
