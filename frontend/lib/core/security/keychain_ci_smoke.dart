...
Future<void> runKeychainCiSmokeTest() async {
  ...
  final random = Random();
  final uuid =
      '${DateTime.now().microsecondsSinceEpoch}_${random.nextInt(100000)}';
  final testKey = 'bdj.license.ci_test.$uuid';

  // Material administrativo ficticio exclusivo para test
  final initialValue =
      'FICTIONAL_OP_KEY_TEST_DATA_${random.nextInt(1000000)}_${DateTime.now().millisecondsSinceEpoch}';
  final updatedValue =
      'FICTIONAL_OP_CERT_TEST_DATA_UPD_${random.nextInt(1000000)}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  void logFailure(String statusToken, String operation, dynamic e) {
    ...
    print('Bundle Identifier: com.bdjstudio.license');
    print(
      'Arquitectura: ${Platform.version} (${Platform.operatingSystemVersion})',
    );
    ...
    if (e is PlatformException) {
      ...
      print(
        'Mensaje sanitizado: ${e.toString().replaceAll(RegExp(r"FICTIONAL_[A-Za-z0-9_]+"), "[REDACTED]")}',
      );
    }
    print('====================================================');
    exit(1);
  ...
  try {
    ...
    if (str.contains('-34018') ||
        str.contains('-25300') ||
        str.contains('item not found') ||
        str.contains('MissingEntitlement')) {
      print('KEYCHAIN_DELETE_PREVIOUS_OK');
    } else {
      logFailure(
        'KEYCHAIN_DELETE_FAILED',
        '1. Eliminar clave temporal anterior',
        e,
      );
    }
  }
  ...
  try {
    ...
    logFailure(
      ...
      '2. Escribir valor temporal aleatorio',
      e,
    );
  }
  ...
  try {
    ...
    if (readVal != initialValue) {
      throw PlatformException(
        ...
        message: 'El valor leído no coincide con el original.',
      );
    }
  ...
  try {
    ...
    if (readUpd != updatedValue) {
      throw PlatformException(
        code: 'OVERWRITE_MISMATCH',
        message: 'El valor leído tras overwrite no coincide.',
      );
    }
  } catch (e) {
  ...
  try {
    ...
    logFailure(
      'KEYCHAIN_PERSISTENCE_FAILED',
      '7. Recrear instancia de almacenamiento',
      e,
    );
  }
  ...
  try {
    ...
    if (readPersisted != updatedValue) {
      throw PlatformException(
        ...
        message: 'Fallo de persistencia al recrear instancia.',
      );
    }
    ...
    logFailure(
      'KEYCHAIN_PERSISTENCE_FAILED',
      '8. Comprobar persistencia tras recreación',
      e,
    );
  }
  ...
  try {
    ...
    if (str.contains('-34018') || str.contains('MissingEntitlement')) {
      ...
      logFailure(
        'KEYCHAIN_DELETE_FAILED',
        '9. Eliminar clave tras persistencia',
        e,
      );
    }
  }
  ...
  try {
    ...
    if (checkNull != null) {
      throw PlatformException(
        ...
        message: 'La clave sigue devolviendo contenido tras delete.',
      );
    }
    ...
    logFailure(
      'KEYCHAIN_DELETE_FAILED',
      '10. Confirmar null en lectura posterior',
      e,
    );
  }
